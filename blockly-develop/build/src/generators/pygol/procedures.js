...
Pygol['procedures_defreturn'] = function (block) {
    ...
    for (let i = 0; i < devVarList.length; i++) {
        globals.push(Pygol.nameDB_.getName(devVarList[i], NameType.DEVELOPER_VARIABLE));
    }
    const globalString = globals.length ?
        Pygol.INDENT + 'global ' + globals.join(', ') + '\n' :
        '';
    const funcName = Pygol.nameDB_.getName(block.getFieldValue('NAME'), NameType.PROCEDURE);
    let xfix1 = '';
    ...
    if (Pygol.INFINITE_LOOP_TRAP) {
        loopTrap = Pygol.prefixLines(Pygol.injectId(Pygol.INFINITE_LOOP_TRAP, block), Pygol.INDENT);
    }
    let branch = Pygol.statementToCode(block, 'STACK');
    let returnValue = Pygol.valueToCode(block, 'RETURN', Pygol.ORDER_NONE) || '';
    let xfix2 = '';
...
Pygol['procedures_ifreturn'] = function (block) {
    ...
    const condition = Pygol.valueToCode(block, 'CONDITION', Pygol.ORDER_NONE) || 'falso';
    let code = 'if ' + condition + ':\n';
    if (Pygol.STATEMENT_SUFFIX) {
        // Inject any statement suffix here since the regular one at the end
        // will not get executed if the return is triggered.
        code += Pygol.prefixLines(Pygol.injectId(Pygol.STATEMENT_SUFFIX, block), Pygol.INDENT);
    }
    if (block.hasReturnValue_) {
        const value = Pygol.valueToCode(block, 'VALUE', Pygol.ORDER_NONE) || 'nulo';
