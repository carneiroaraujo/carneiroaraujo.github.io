...
Pygol['controls_whileUntil'] = function (block) {
    ...
    let argument0 = Pygol.valueToCode(block, 'BOOL', until ? Pygol.ORDER_LOGICAL_NOT : Pygol.ORDER_NONE) ||
        'falso';
    let branch = Pygol.statementToCode(block, 'DO');
    branch = Pygol.addLoopTrap(branch, block) || Pygol.PASS;
    if (until) {
...
Pygol['controls_for'] = function (block) {
    // For loop.
    const variable0 = Pygol.nameDB_.getName(block.getFieldValue('VAR'), NameType.VARIABLE);
    ...
    if (stringUtils.isNumber(argument0) && stringUtils.isNumber(argument1) &&
        ...
        const scrub = function (arg, suffix) {
            if (stringUtils.isNumber(arg)) {
                ...
                const varName = Pygol.nameDB_.getDistinctName(variable0 + suffix, NameType.VARIABLE);
                code += varName + ' = ' + arg + '\n';
                arg = varName;
            }
            return arg;
...
Pygol['controls_forEach'] = function (block) {
    // For each loop.
    const variable0 = Pygol.nameDB_.getName(block.getFieldValue('VAR'), NameType.VARIABLE);
    const argument0 = Pygol.valueToCode(block, 'LIST', Pygol.ORDER_RELATIONAL) || '[]';
    let branch = Pygol.statementToCode(block, 'DO');
    branch = Pygol.addLoopTrap(branch, block) || Pygol.PASS;
