...
Pygol['lists_repeat'] = function (block) {
    ...
    const times = Pygol.valueToCode(block, 'NUM', Pygol.ORDER_MULTIPLICATIVE) || '0';
...
Pygol['lists_getIndex'] = function (block) {
    ...
    const listOrder = (where === 'RANDOM') ? Pygol.ORDER_NONE : Pygol.ORDER_MEMBER;
    ...
    switch (where) {
        ...
        case 'RANDOM':
            ...
            if (mode === 'GET') {
                ...
                const functionName = Pygol.provideFunction_('lists_remove_random_item', `
...
Pygol['lists_setIndex'] = function (block) {
    ...
    function cacheList() {
        ...
        const listVar = Pygol.nameDB_.getDistinctName('tmp_list', NameType.VARIABLE);
...
Pygol['lists_split'] = function (block) {
    ...
    let code;
    if (mode === 'SPLIT') {
        const value_input = Pygol.valueToCode(block, 'INPUT', Pygol.ORDER_MEMBER) || "''";
        const value_delim = Pygol.valueToCode(block, 'DELIM', Pygol.ORDER_NONE);
        code = value_input + '.split(' + value_delim + ')';
    }
    else if (mode === 'JOIN') {
        const value_input = Pygol.valueToCode(block, 'INPUT', Pygol.ORDER_NONE) || '[]';
        const value_delim = Pygol.valueToCode(block, 'DELIM', Pygol.ORDER_MEMBER) || "''";
