...
Pygol['text_multiline'] = function (block) {
    ...
    const order = code.indexOf('+') !== -1 ? Pygol.ORDER_ADDITIVE : Pygol.ORDER_ATOMIC;
    return [code, order];
};
/**
...
Pygol['text_join'] = function (block) {
    ...
    switch (block.itemCount_) {
        ...
        case 1: {
            const element = Pygol.valueToCode(block, 'ADD0', Pygol.ORDER_NONE) || "''";
            const codeAndOrder = forceString(element);
            return codeAndOrder;
        }
        case 2: {
            const element0 = Pygol.valueToCode(block, 'ADD0', Pygol.ORDER_NONE) || "''";
            const element1 = Pygol.valueToCode(block, 'ADD1', Pygol.ORDER_NONE) || "''";
...
Pygol['text_indexOf'] = function (block) {
    ...
    const substring = Pygol.valueToCode(block, 'FIND', Pygol.ORDER_NONE) || "''";
    const text = Pygol.valueToCode(block, 'VALUE', Pygol.ORDER_MEMBER) || "''";
...
Pygol['text_charAt'] = function (block) {
    ...
    const textOrder = (where === 'RANDOM') ? Pygol.ORDER_NONE : Pygol.ORDER_MEMBER;
...
Pygol['text_getSubstring'] = function (block) {
    ...
    const text = Pygol.valueToCode(block, 'STRING', Pygol.ORDER_MEMBER) || "''";
    let at1;
    switch (where1) {
        case 'FROM_START':
