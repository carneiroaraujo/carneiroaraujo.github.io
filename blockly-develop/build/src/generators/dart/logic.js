...
Dart['controls_if'] = function (block) {
    ...
    do {
        ...
        if (Dart.STATEMENT_SUFFIX) {
            branchCode =
                Dart.prefixLines(Dart.injectId(Dart.STATEMENT_SUFFIX, block), Dart.INDENT) +
                    branchCode;
        }
...
Dart['logic_operation'] = function (block) {
    ...
    const order = (operator === '&&') ? Dart.ORDER_LOGICAL_AND : Dart.ORDER_LOGICAL_OR;
...
Dart['logic_ternary'] = function (block) {
    // Ternary operator.
    const value_if = Dart.valueToCode(block, 'IF', Dart.ORDER_CONDITIONAL) || 'false';
    const value_then = Dart.valueToCode(block, 'THEN', Dart.ORDER_CONDITIONAL) || 'null';
    const value_else = Dart.valueToCode(block, 'ELSE', Dart.ORDER_CONDITIONAL) || 'null';
