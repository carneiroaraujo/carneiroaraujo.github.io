...
Dart['text_multiline'] = function (block) {
    ...
    const order = code.indexOf('+') !== -1 ? Dart.ORDER_ADDITIVE : Dart.ORDER_ATOMIC;
    return [code, order];
};
Dart['text_join'] = function (block) {
    ...
    switch (block.itemCount_) {
        ...
        case 1: {
            const element = Dart.valueToCode(block, 'ADD0', Dart.ORDER_UNARY_POSTFIX) || "''";
...
Dart['text_length'] = function (block) {
    ...
    const text = Dart.valueToCode(block, 'VALUE', Dart.ORDER_UNARY_POSTFIX) || "''";
...
Dart['text_getSubstring'] = function (block) {
    ...
    const textOrder = requiresLengthCall ? Dart.ORDER_UNARY_POSTFIX : Dart.ORDER_NONE;
...
Dart['text_trim'] = function (block) {
    ...
    const text = Dart.valueToCode(block, 'TEXT', Dart.ORDER_UNARY_POSTFIX) || "''";
