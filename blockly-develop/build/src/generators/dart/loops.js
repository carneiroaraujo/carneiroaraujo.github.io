...
Dart['controls_whileUntil'] = function (block) {
    ...
    let argument0 = Dart.valueToCode(block, 'BOOL', until ? Dart.ORDER_UNARY_PREFIX : Dart.ORDER_NONE) ||
        'false';
    let branch = Dart.statementToCode(block, 'DO');
    branch = Dart.addLoopTrap(branch, block);
    if (until) {
...
Dart['controls_for'] = function (block) {
    // For loop.
    const variable0 = Dart.nameDB_.getName(block.getFieldValue('VAR'), NameType.VARIABLE);
    const argument0 = Dart.valueToCode(block, 'FROM', Dart.ORDER_ASSIGNMENT) || '0';
    ...
    if (stringUtils.isNumber(argument0) && stringUtils.isNumber(argument1) &&
        ...
        const incVar = Dart.nameDB_.getDistinctName(variable0 + '_inc', NameType.VARIABLE);
        code += 'num ' + incVar + ' = ';
...
Dart['controls_forEach'] = function (block) {
    // For each loop.
    const variable0 = Dart.nameDB_.getName(block.getFieldValue('VAR'), NameType.VARIABLE);
    const argument0 = Dart.valueToCode(block, 'LIST', Dart.ORDER_ASSIGNMENT) || '[]';
    let branch = Dart.statementToCode(block, 'DO');
    branch = Dart.addLoopTrap(branch, block);
