...
Dart['procedures_defreturn'] = function (block) {
    ...
    const funcName = Dart.nameDB_.getName(block.getFieldValue('NAME'), NameType.PROCEDURE);
    let xfix1 = '';
    ...
    if (Dart.INFINITE_LOOP_TRAP) {
        loopTrap = Dart.prefixLines(Dart.injectId(Dart.INFINITE_LOOP_TRAP, block), Dart.INDENT);
    }
...
Dart['procedures_ifreturn'] = function (block) {
    ...
    const condition = Dart.valueToCode(block, 'CONDITION', Dart.ORDER_NONE) || 'false';
    let code = 'if (' + condition + ') {\n';
    if (Dart.STATEMENT_SUFFIX) {
        // Inject any statement suffix here since the regular one at the end
        // will not get executed if the return is triggered.
        code += Dart.prefixLines(Dart.injectId(Dart.STATEMENT_SUFFIX, block), Dart.INDENT);
    }
