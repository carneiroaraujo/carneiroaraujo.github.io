...
Python['procedures_defreturn'] = function (block) {
    ...
    for (let i = 0; i < devVarList.length; i++) {
        globals.push(Python.nameDB_.getName(devVarList[i], NameType.DEVELOPER_VARIABLE));
    }
    ...
    if (Python.INFINITE_LOOP_TRAP) {
        loopTrap = Python.prefixLines(Python.injectId(Python.INFINITE_LOOP_TRAP, block), Python.INDENT);
    }
...
Python['procedures_ifreturn'] = function (block) {
    ...
    const condition = Python.valueToCode(block, 'CONDITION', Python.ORDER_NONE) || 'falso';
    let code = 'if ' + condition + ':\n';
    if (Python.STATEMENT_SUFFIX) {
        // Inject any statement suffix here since the regular one at the end
        // will not get executed if the return is triggered.
        code += Python.prefixLines(Python.injectId(Python.STATEMENT_SUFFIX, block), Python.INDENT);
    }
    if (block.hasReturnValue_) {
        const value = Python.valueToCode(block, 'VALUE', Python.ORDER_NONE) || 'nulo';
