...
Python['lists_repeat'] = function (block) {
    ...
    const times = Python.valueToCode(block, 'NUM', Python.ORDER_MULTIPLICATIVE) || '0';
...
Python['lists_getIndex'] = function (block) {
    ...
    const listOrder = (where === 'RANDOM') ? Python.ORDER_NONE : Python.ORDER_MEMBER;
    ...
    switch (where) {
        ...
        case 'RANDOM':
            ...
            if (mode === 'GET') {
                ...
                const functionName = Python.provideFunction_('lists_remove_random_item', `
...
Python['lists_setIndex'] = function (block) {
    ...
    function cacheList() {
        ...
        const listVar = Python.nameDB_.getDistinctName('tmp_list', NameType.VARIABLE);
...
Python['lists_split'] = function (block) {
    ...
    let code;
    if (mode === 'SPLIT') {
        const value_input = Python.valueToCode(block, 'INPUT', Python.ORDER_MEMBER) || "''";
        const value_delim = Python.valueToCode(block, 'DELIM', Python.ORDER_NONE);
        code = value_input + '.split(' + value_delim + ')';
    }
    else if (mode === 'JOIN') {
        const value_input = Python.valueToCode(block, 'INPUT', Python.ORDER_NONE) || '[]';
        const value_delim = Python.valueToCode(block, 'DELIM', Python.ORDER_MEMBER) || "''";
