...
PHP['controls_if'] = function (block) {
    ...
    do {
        ...
        if (PHP.STATEMENT_SUFFIX) {
            branchCode = PHP.prefixLines(PHP.injectId(PHP.STATEMENT_SUFFIX, block), PHP.INDENT) +
                branchCode;
        }
...
PHP['logic_operation'] = function (block) {
    ...
    const order = (operator === '&&') ? PHP.ORDER_LOGICAL_AND : PHP.ORDER_LOGICAL_OR;
...
PHP['logic_ternary'] = function (block) {
    // Ternary operator.
    const value_if = PHP.valueToCode(block, 'IF', PHP.ORDER_CONDITIONAL) || 'false';
    const value_then = PHP.valueToCode(block, 'THEN', PHP.ORDER_CONDITIONAL) || 'null';
    const value_else = PHP.valueToCode(block, 'ELSE', PHP.ORDER_CONDITIONAL) || 'null';
