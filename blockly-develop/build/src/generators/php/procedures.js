...
PHP['procedures_defreturn'] = function (block) {
    ...
    for (let i = 0; i < devVarList.length; i++) {
        globals.push(PHP.nameDB_.getName(devVarList[i], NameType.DEVELOPER_VARIABLE));
    }
    const globalStr = globals.length ? PHP.INDENT + 'global ' + globals.join(', ') + ';\n' : '';
    const funcName = PHP.nameDB_.getName(block.getFieldValue('NAME'), NameType.PROCEDURE);
    let xfix1 = '';
    ...
    if (PHP.INFINITE_LOOP_TRAP) {
        loopTrap = PHP.prefixLines(PHP.injectId(PHP.INFINITE_LOOP_TRAP, block), PHP.INDENT);
    }
...
PHP['procedures_ifreturn'] = function (block) {
    ...
    const condition = PHP.valueToCode(block, 'CONDITION', PHP.ORDER_NONE) || 'false';
