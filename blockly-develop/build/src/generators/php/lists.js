...
PHP['lists_isEmpty'] = function (block) {
    ...
    const argument0 = PHP.valueToCode(block, 'VALUE', PHP.ORDER_FUNCTION_CALL) || 'array()';
...
PHP['lists_getIndex'] = function (block) {
    ...
    switch (where) {
        case 'FIRST':
            if (mode === 'GET') {
                const list = PHP.valueToCode(block, 'VALUE', PHP.ORDER_MEMBER) || 'array()';
                const code = list + '[0]';
        ...
        case 'FROM_END':
            if (mode === 'GET') {
                ...
                const list = PHP.valueToCode(block, 'VALUE', PHP.ORDER_NONE) || 'array()';
                const at = PHP.getAdjusted(block, 'AT', 1, false, PHP.ORDER_SUBTRACTION);
...
`);
                ...
                const functionName = PHP.provideFunction_('lists_get_remove_random_item', `
function ${PHP.FUNCTION_NAME_PLACEHOLDER_}(&$list) {
...
PHP['lists_setIndex'] = function (block) {
    ...
    switch (where) {
        case 'FIRST':
            if (mode === 'SET') {
                const list = PHP.valueToCode(block, 'LIST', PHP.ORDER_MEMBER) || 'array()';
                return list + '[0] = ' + value + ';\n';
            }
            else if (mode === 'INSERT') {
