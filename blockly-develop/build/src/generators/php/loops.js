...
PHP['controls_whileUntil'] = function (block) {
    ...
    let argument0 = PHP.valueToCode(block, 'BOOL', until ? PHP.ORDER_LOGICAL_NOT : PHP.ORDER_NONE) ||
        'false';
    let branch = PHP.statementToCode(block, 'DO');
    branch = PHP.addLoopTrap(branch, block);
    if (until) {
...
PHP['controls_for'] = function (block) {
    // For loop.
    const variable0 = PHP.nameDB_.getName(block.getFieldValue('VAR'), NameType.VARIABLE);
    ...
    if (stringUtils.isNumber(argument0) && stringUtils.isNumber(argument1) &&
        ...
        const incVar = PHP.nameDB_.getDistinctName(variable0 + '_inc', NameType.VARIABLE);
        code += incVar + ' = ';
...
PHP['controls_forEach'] = function (block) {
    // For each loop.
    const variable0 = PHP.nameDB_.getName(block.getFieldValue('VAR'), NameType.VARIABLE);
    const argument0 = PHP.valueToCode(block, 'LIST', PHP.ORDER_ASSIGNMENT) || '[]';
    let branch = PHP.statementToCode(block, 'DO');
    branch = PHP.addLoopTrap(branch, block);
    let code = '';
    code +=
