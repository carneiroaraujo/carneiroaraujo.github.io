...
PHP['text_multiline'] = function (block) {
    ...
    const order = code.indexOf('.') !== -1 ? PHP.ORDER_STRING_CONCAT : PHP.ORDER_ATOMIC;
    return [code, order];
};
PHP['text_join'] = function (block) {
    ...
    if (block.itemCount_ === 0) {
        ...
        const element0 = PHP.valueToCode(block, 'ADD0', PHP.ORDER_STRING_CONCAT) || "''";
        const element1 = PHP.valueToCode(block, 'ADD1', PHP.ORDER_STRING_CONCAT) || "''";
...
PHP['text_indexOf'] = function (block) {
    ...
    const functionName = PHP.provideFunction_(block.getFieldValue('END') === 'FIRST' ? 'text_indexOf' :
        'text_lastIndexOf', `
