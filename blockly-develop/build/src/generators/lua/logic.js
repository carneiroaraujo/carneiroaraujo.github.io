...
Lua['controls_if'] = function (block) {
    ...
    do {
        const conditionCode = Lua.valueToCode(block, 'IF' + n, Lua.ORDER_NONE) || 'false';
        let branchCode = Lua.statementToCode(block, 'DO' + n);
        if (Lua.STATEMENT_SUFFIX) {
            branchCode = Lua.prefixLines(Lua.injectId(Lua.STATEMENT_SUFFIX, block), Lua.INDENT) + branchCode;
        }
        code +=
    ...
    if (block.getInput('ELSE') || Lua.STATEMENT_SUFFIX) {
        ...
        if (Lua.STATEMENT_SUFFIX) {
            branchCode = Lua.prefixLines(Lua.injectId(Lua.STATEMENT_SUFFIX, block), Lua.INDENT) +
                branchCode;
        }
        code += 'else\n' + branchCode;
    }
