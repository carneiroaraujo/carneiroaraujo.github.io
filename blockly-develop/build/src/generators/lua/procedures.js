...
Lua['procedures_defreturn'] = function (block) {
    ...
    const funcName = Lua.nameDB_.getName(block.getFieldValue('NAME'), NameType.PROCEDURE);
    let xfix1 = '';
    ...
    if (Lua.INFINITE_LOOP_TRAP) {
        loopTrap = Lua.prefixLines(Lua.injectId(Lua.INFINITE_LOOP_TRAP, block), Lua.INDENT);
    }
...
Lua['procedures_ifreturn'] = function (block) {
    ...
    const condition = Lua.valueToCode(block, 'CONDITION', Lua.ORDER_NONE) || 'false';
