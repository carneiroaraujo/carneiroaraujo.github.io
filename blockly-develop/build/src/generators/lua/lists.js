...
Lua['lists_getIndex'] = function (block) {
    ...
    if ((where === 'LAST' || where === 'FROM_END' || where === 'RANDOM') &&
        ...
        if (mode === 'REMOVE') {
            ...
            const atOrder = (where === 'FROM_END') ? Lua.ORDER_ADDITIVE : Lua.ORDER_NONE;
...
Lua['lists_getSublist'] = function (block) {
    ...
    const functionName = Lua.provideFunction_('list_sublist_' + where1.toLowerCase() + '_' + where2.toLowerCase(), `
