...
Lua['math_number_property'] = function (block) {
    ...
    const numberToCheck = Lua.valueToCode(block, 'NUMBER_TO_CHECK', inputOrder) || '0';
    let code;
...
`);
        ...
        const divisor = Lua.valueToCode(block, 'DIVISOR', Lua.ORDER_MULTIPLICATIVE) || '0';
...
Lua['math_change'] = function (block) {
    ...
    const varName = Lua.nameDB_.getName(block.getFieldValue('VAR'), NameType.VARIABLE);
...
Lua['math_modulo'] = function (block) {
    ...
    const argument0 = Lua.valueToCode(block, 'DIVIDEND', Lua.ORDER_MULTIPLICATIVE) || '0';
    const argument1 = Lua.valueToCode(block, 'DIVISOR', Lua.ORDER_MULTIPLICATIVE) || '0';
...
Lua['math_constrain'] = function (block) {
    ...
    const argument1 = Lua.valueToCode(block, 'LOW', Lua.ORDER_NONE) || '-math.huge';
    const argument2 = Lua.valueToCode(block, 'HIGH', Lua.ORDER_NONE) || 'math.huge';
