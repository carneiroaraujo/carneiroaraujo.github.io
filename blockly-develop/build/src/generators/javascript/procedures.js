...
JavaScript['procedures_defreturn'] = function (block) {
    ...
    const funcName = JavaScript.nameDB_.getName(block.getFieldValue('NAME'), NameType.PROCEDURE);
    let xfix1 = '';
    ...
    if (JavaScript.INFINITE_LOOP_TRAP) {
        loopTrap = JavaScript.prefixLines(JavaScript.injectId(JavaScript.INFINITE_LOOP_TRAP, block), JavaScript.INDENT);
    }
    const branch = JavaScript.statementToCode(block, 'STACK');
    let returnValue = JavaScript.valueToCode(block, 'RETURN', JavaScript.ORDER_NONE) || '';
    let xfix2 = '';
...
JavaScript['procedures_ifreturn'] = function (block) {
    ...
    const condition = JavaScript.valueToCode(block, 'CONDITION', JavaScript.ORDER_NONE) ||
        'false';
    let code = 'if (' + condition + ') {\n';
    if (JavaScript.STATEMENT_SUFFIX) {
        // Inject any statement suffix here since the regular one at the end
        // will not get executed if the return is triggered.
        code += JavaScript.prefixLines(JavaScript.injectId(JavaScript.STATEMENT_SUFFIX, block), JavaScript.INDENT);
    }
    if (block.hasReturnValue_) {
        const value = JavaScript.valueToCode(block, 'VALUE', JavaScript.ORDER_NONE) || 'null';
