...
`);
    const element = JavaScript.valueToCode(block, 'ITEM', JavaScript.ORDER_NONE) || 'null';
    const repeatCount = JavaScript.valueToCode(block, 'NUM', JavaScript.ORDER_NONE) || '0';
...
JavaScript['lists_getIndex'] = function (block) {
    ...
    const listOrder = (where === 'RANDOM') ? JavaScript.ORDER_NONE : JavaScript.ORDER_MEMBER;
...
JavaScript['lists_setIndex'] = function (block) {
    ...
    let list = JavaScript.valueToCode(block, 'LIST', JavaScript.ORDER_MEMBER) || '[]';
    const mode = block.getFieldValue('MODE') || 'GET';
    const where = block.getFieldValue('WHERE') || 'FROM_START';
    const value = JavaScript.valueToCode(block, 'TO', JavaScript.ORDER_ASSIGNMENT) ||
        'null';
    ...
    function cacheList() {
        ...
        const listVar = JavaScript.nameDB_.getDistinctName('tmpList', NameType.VARIABLE);
    ...
    switch (where) {
        ...
        case ('FROM_END'): {
            const at = JavaScript.getAdjusted(block, 'AT', 1, false, JavaScript.ORDER_SUBTRACTION);
            let code = cacheList();
            if (mode === 'SET') {
        ...
        case ('RANDOM'): {
            let code = cacheList();
            const xVar = JavaScript.nameDB_.getDistinctName('tmpX', NameType.VARIABLE);
...
JavaScript['lists_getSublist'] = function (block) {
    // Get sublist.
    const list = JavaScript.valueToCode(block, 'LIST', JavaScript.ORDER_MEMBER) || '[]';
    ...
    if (where1 === 'FIRST' && where2 === 'LAST') {
        ...
        switch (where1) {
            case 'FROM_START':
                ...
                break;
            case 'FROM_END':
                at1 = JavaScript.getAdjusted(block, 'AT1', 1, false, JavaScript.ORDER_SUBTRACTION);
                at1 = list + '.length - ' + at1;
                break;
            case 'FIRST':
                at1 = '0';
        ...
        switch (where2) {
            case 'FROM_START':
                ...
                break;
            case 'FROM_END':
                at2 = JavaScript.getAdjusted(block, 'AT2', 0, false, JavaScript.ORDER_SUBTRACTION);
                at2 = list + '.length - ' + at2;
                break;
            case 'LAST':
                at2 = list + '.length';
        ...
        const functionName = JavaScript.provideFunction_('subsequence' + wherePascalCase[where1] + wherePascalCase[where2], `
...
JavaScript['lists_sort'] = function (block) {
    ...
    const list = JavaScript.valueToCode(block, 'LIST', JavaScript.ORDER_FUNCTION_CALL) ||
        '[]';
    const direction = block.getFieldValue('DIRECTION') === '1' ? 1 : -1;
    const type = block.getFieldValue('TYPE');
    const getCompareFunctionName = JavaScript.provideFunction_('listsGetSortCompare', `
...
JavaScript['lists_split'] = function (block) {
    ...
    const delimiter = JavaScript.valueToCode(block, 'DELIM', JavaScript.ORDER_NONE) || "''";
