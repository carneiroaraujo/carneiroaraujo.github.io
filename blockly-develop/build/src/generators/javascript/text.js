...
JavaScript['text_join'] = function (block) {
    ...
    switch (block.itemCount_) {
        ...
        case 1: {
            const element = JavaScript.valueToCode(block, 'ADD0', JavaScript.ORDER_NONE) || "''";
            const codeAndOrder = forceString(element);
            return codeAndOrder;
        }
        case 2: {
            const element0 = JavaScript.valueToCode(block, 'ADD0', JavaScript.ORDER_NONE) || "''";
            const element1 = JavaScript.valueToCode(block, 'ADD1', JavaScript.ORDER_NONE) || "''";
        ...
        default: {
            ...
            for (let i = 0; i < block.itemCount_; i++) {
                elements[i] = JavaScript.valueToCode(block, 'ADD' + i, JavaScript.ORDER_NONE) || "''";
            }
...
JavaScript['text_append'] = function (block) {
    ...
    const value = JavaScript.valueToCode(block, 'TEXT', JavaScript.ORDER_NONE) || "''";
    const code = varName + ' += ' +
        forceString(value)[0] + ';\n';
    return code;
};
JavaScript['text_length'] = function (block) {
    ...
    const text = JavaScript.valueToCode(block, 'VALUE', JavaScript.ORDER_MEMBER) || "''";
...
JavaScript['text_indexOf'] = function (block) {
    ...
    const operator = block.getFieldValue('END') === 'FIRST' ?
        'indexOf' : 'lastIndexOf';
    const substring = JavaScript.valueToCode(block, 'FIND', JavaScript.ORDER_NONE) || "''";
    const text = JavaScript.valueToCode(block, 'VALUE', JavaScript.ORDER_MEMBER) || "''";
...
JavaScript['text_trim'] = function (block) {
    ...
    const text = JavaScript.valueToCode(block, 'TEXT', JavaScript.ORDER_MEMBER) || "''";
    return [text + operator, JavaScript.ORDER_FUNCTION_CALL];
};
JavaScript['text_print'] = function (block) {
    // Print statement.
    const msg = JavaScript.valueToCode(block, 'TEXT', JavaScript.ORDER_NONE) || "''";
...
JavaScript['text_count'] = function (block) {
    ...
    const sub = JavaScript.valueToCode(block, 'SUB', JavaScript.ORDER_NONE) || "''";
...
JavaScript['text_replace'] = function (block) {
    const text = JavaScript.valueToCode(block, 'TEXT', JavaScript.ORDER_NONE) || "''";
    const from = JavaScript.valueToCode(block, 'FROM', JavaScript.ORDER_NONE) || "''";
