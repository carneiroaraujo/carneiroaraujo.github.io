...
JavaScript['controls_repeat_ext'] = function (block) {
    ...
    let code = '';
    const loopVar = JavaScript.nameDB_.getDistinctName('count', NameType.VARIABLE);
    let endVar = repeats;
...
JavaScript['controls_whileUntil'] = function (block) {
    ...
    let argument0 = JavaScript.valueToCode(block, 'BOOL', until ? JavaScript.ORDER_LOGICAL_NOT : JavaScript.ORDER_NONE) ||
        'false';
    let branch = JavaScript.statementToCode(block, 'DO');
    branch = JavaScript.addLoopTrap(branch, block);
    if (until) {
...
JavaScript['controls_for'] = function (block) {
    ...
    const argument0 = JavaScript.valueToCode(block, 'FROM', JavaScript.ORDER_ASSIGNMENT) || '0';
    const argument1 = JavaScript.valueToCode(block, 'TO', JavaScript.ORDER_ASSIGNMENT) || '0';
    const increment = JavaScript.valueToCode(block, 'BY', JavaScript.ORDER_ASSIGNMENT) || '1';
    let branch = JavaScript.statementToCode(block, 'DO');
    branch = JavaScript.addLoopTrap(branch, block);
    let code;
    if (stringUtils.isNumber(argument0) && stringUtils.isNumber(argument1) &&
        ...
        if (!argument0.match(/^\w+$/) && !stringUtils.isNumber(argument0)) {
            startVar = JavaScript.nameDB_.getDistinctName(variable0 + '_start', NameType.VARIABLE);
            code += 'var ' + startVar + ' = ' + argument0 + ';\n';
        }
        let endVar = argument1;
        if (!argument1.match(/^\w+$/) && !stringUtils.isNumber(argument1)) {
            endVar = JavaScript.nameDB_.getDistinctName(variable0 + '_end', NameType.VARIABLE);
            code += 'var ' + endVar + ' = ' + argument1 + ';\n';
        }
        // Determine loop direction at start, in case one of the bounds
        // changes during loop execution.
        const incVar = JavaScript.nameDB_.getDistinctName(variable0 + '_inc', NameType.VARIABLE);
        code += 'var ' + incVar + ' = ';
...
JavaScript['controls_forEach'] = function (block) {
    ...
    if (!argument0.match(/^\w+$/)) {
        listVar = JavaScript.nameDB_.getDistinctName(variable0 + '_list', NameType.VARIABLE);
        code += 'var ' + listVar + ' = ' + argument0 + ';\n';
    }
    const indexVar = JavaScript.nameDB_.getDistinctName(variable0 + '_index', NameType.VARIABLE);
