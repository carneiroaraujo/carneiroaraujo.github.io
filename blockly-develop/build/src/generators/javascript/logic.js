...
JavaScript['controls_if'] = function (block) {
    ...
    do {
        const conditionCode = JavaScript.valueToCode(block, 'IF' + n, JavaScript.ORDER_NONE) ||
            'false';
        let branchCode = JavaScript.statementToCode(block, 'DO' + n);
        if (JavaScript.STATEMENT_SUFFIX) {
            branchCode = JavaScript.prefixLines(JavaScript.injectId(JavaScript.STATEMENT_SUFFIX, block), JavaScript.INDENT) +
                branchCode;
        }
...
JavaScript['logic_ternary'] = function (block) {
    // Ternary operator.
    const value_if = JavaScript.valueToCode(block, 'IF', JavaScript.ORDER_CONDITIONAL) ||
        'false';
    const value_then = JavaScript.valueToCode(block, 'THEN', JavaScript.ORDER_CONDITIONAL) ||
        'null';
    const value_else = JavaScript.valueToCode(block, 'ELSE', JavaScript.ORDER_CONDITIONAL) ||
        'null';
