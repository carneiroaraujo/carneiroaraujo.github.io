...
class FieldDropdown extends Field {
    ...
    createTextArrow_() {
        ...
        this.arrow_.appendChild(document.createTextNode(this.getSourceBlock()?.RTL ? FieldDropdown.ARROW_CHAR + ' ' :
    ...
    createSVGArrow_() {
        this.svgArrow_ = dom.createSvgElement(Svg.IMAGE, {
            'height': this.getConstants().FIELD_DROPDOWN_SVG_ARROW_SIZE + 'px',
            'width': this.getConstants().FIELD_DROPDOWN_SVG_ARROW_SIZE + 'px',
        }, this.fieldGroup_);
        this.svgArrow_.setAttributeNS(dom.XLINK_NS, 'xlink:href', this.getConstants().FIELD_DROPDOWN_SVG_ARROW_DATAURI);
    }
    /**
    ...
    showEditor_(e) {
        ...
        if (this.getConstants().FIELD_DROPDOWN_COLOURED_DIV) {
            const primaryColour = block.isShadow() ? block.getParent().getColour() : block.getColour();
        ...
        if (this.selectedMenuItem_) {
            ...
            style.scrollIntoContainerView(this.selectedMenuItem_.getElement(), dropDownDiv.getContentDiv(), true);
        }
        this.applyColour();
    }
    ...
    renderSelectedImage_(imageJson) {
        ...
        if (this.svgArrow_) {
            ...
            arrowWidth = dom.getFastTextWidth(this.arrow_, this.getConstants().FIELD_TEXT_FONTSIZE, this.getConstants().FIELD_TEXT_FONTWEIGHT, this.getConstants().FIELD_TEXT_FONTFAMILY);
        }
...
function applyTrim(options, prefixLength, suffixLength) {
    return options.map(([text, value]) => [text.substring(prefixLength, text.length - suffixLength),
        value,
    ]);
}
/**
...
function validateOptions(options) {
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        if (!Array.isArray(tuple)) {
            foundError = true;
            console.error('Invalid option[' + i + ']: Each FieldDropdown option must be an ' +
                'array. Found: ', tuple);
        }
        else if (typeof tuple[1] !== 'string') {
            foundError = true;
            console.error('Invalid option[' + i + ']: Each FieldDropdown option id must be ' +
                'a string. Found ' + tuple[1] + ' in: ', tuple);
        }
        else if (tuple[0] && typeof tuple[0] !== 'string' &&
            typeof tuple[0].src !== 'string') {
            foundError = true;
            console.error('Invalid option[' + i + ']: Each FieldDropdown option must have a ' +
                'string label or image description. Found' + tuple[0] + ' in: ', tuple);
        }
    }
    if (foundError) {
