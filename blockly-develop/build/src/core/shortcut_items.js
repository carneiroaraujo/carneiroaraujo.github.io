...
export function registerCopy() {
    ...
    const copyShortcut = {
        ...
        callback(workspace, e) {
            ...
            // type 'Workspace'.
            workspace.hideChaff();
            clipboard.copy(common.getSelected());
            return true;
        },
...
export function registerCut() {
    const ctrlX = ShortcutRegistry.registry.createSerializedKey(KeyCodes.X, [KeyCodes.CTRL]);
    const altX = ShortcutRegistry.registry.createSerializedKey(KeyCodes.X, [KeyCodes.ALT]);
    const metaX = ShortcutRegistry.registry.createSerializedKey(KeyCodes.X, [KeyCodes.META]);
    const cutShortcut = {
        name: names.CUT,
        preconditionFn(workspace) {
            const selected = common.getSelected();
...
export function registerPaste() {
    ...
    const altV = ShortcutRegistry.registry.createSerializedKey(KeyCodes.V, [KeyCodes.ALT]);
    const metaV = ShortcutRegistry.registry.createSerializedKey(KeyCodes.V, [KeyCodes.META]);
    const pasteShortcut = {
        name: names.PASTE,
        preconditionFn(workspace) {
            return !workspace.options.readOnly && !Gesture.inProgress();
...
export function registerRedo() {
    const ctrlShiftZ = ShortcutRegistry.registry.createSerializedKey(KeyCodes.Z, [KeyCodes.SHIFT, KeyCodes.CTRL]);
    const altShiftZ = ShortcutRegistry.registry.createSerializedKey(KeyCodes.Z, [KeyCodes.SHIFT, KeyCodes.ALT]);
    const metaShiftZ = ShortcutRegistry.registry.createSerializedKey(KeyCodes.Z, [KeyCodes.SHIFT, KeyCodes.META]);
    // Ctrl-y is redo in Windows.  Command-y is never valid on Macs.
    const ctrlY = ShortcutRegistry.registry.createSerializedKey(KeyCodes.Y, [KeyCodes.CTRL]);
    const redoShortcut = {
        name: names.REDO,
        preconditionFn(workspace) {
