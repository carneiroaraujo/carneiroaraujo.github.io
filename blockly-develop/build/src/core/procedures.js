...
export function allProcedures(root) {
    const proceduresNoReturn = root.getProcedureMap()
        .getProcedures()
    ...
    const proceduresReturn = root.getProcedureMap()
        .getProcedures()
        .filter((p) => !!p.getReturnTypes())
        .map((p) => [p.getName(),
        p.getParameters().map((pa) => pa.getName()),
...
export function mutatorOpenListener(e) {
    ...
    const block = common.getWorkspaceById(workspaceId).getBlockById(bubbleEvent.blockId);
