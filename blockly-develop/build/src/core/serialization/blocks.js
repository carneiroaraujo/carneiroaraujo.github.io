 ...
 */
export function save(block, { addCoordinates = false, addInputBlocks = true, addNextBlocks = true, doFullSerialization = true, } = {}) {
 ...
 */
function saveInputBlocks(block, state, doFullSerialization) {
    ...
    for (let i = 0; i < block.inputList.length; i++) {
        ...
        const connectionState = saveConnection(input.connection, doFullSerialization);
        if (connectionState) {
...
function saveNextBlocks(block, state, doFullSerialization) {
    ...
    const connectionState = saveConnection(block.nextConnection, doFullSerialization);
    if (connectionState) {
...
function tryToConnectParent(parentConnection, child, state) {
    ...
    if (!connected) {
        ...
        throw new BadConnectionCheck(checker.getErrorMessage(checker.canConnectWithReason(childConnection, parentConnection, false), childConnection, parentConnection), parentConnection.type === inputTypes.VALUE ? 'output connection' :
...
export class BlockSerializer {
    ...
    save(workspace) {
        ...
        for (const block of workspace.getTopBlocks(false)) {
            const state = saveBlock(block, { addCoordinates: true, doFullSerialization: false });
            if (state) {
