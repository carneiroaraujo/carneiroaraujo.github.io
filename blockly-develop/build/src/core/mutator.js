...
export class Mutator extends Icon {
    ...
    constructor(quarkNames, block) {
        if (!block) {
            deprecation.warn('Calling the Mutator constructor without passing the block it is attached to', 'version 9', 'version 10', 'the constructor by passing the list of subblocks and the block instance to attach the mutator to');
        }
        super(block ?? null);
        /**
    ...
    drawIcon_(group) {
        ...
        // Axle hole.
        dom.createSvgElement(Svg.CIRCLE, { 'class': 'blocklyIconShape', 'r': '2.7', 'cx': '8', 'cy': '8' }, group);
    }
    /**
    ...
    createEditor() {
        /* Create the editor.  Here's the markup that will be generated:
            ...
            */
        this.svgDialog = dom.createSvgElement(Svg.SVG, { 'x': Bubble.BORDER_WIDTH, 'y': Bubble.BORDER_WIDTH });
    ...
    resizeBubble() {
        ...
        if (flyout) {
            const flyoutScrollMetrics = flyout.getWorkspace().getMetricsManager().getScrollMetrics();
    ...
    setVisible(visible) {
        ...
        if (visible) {
            // Create the bubble.
            this.bubble_ = new Bubble(block.workspace, this.createEditor(), block.pathObject.svgPath, this.iconXY_, null, null);
            ...
            if (flyout)
                flyout.getWorkspace().addChangeListener(boundListener);
        ...
        eventUtils.fire(new (eventUtils.get(eventUtils.BUBBLE_OPEN))(block, visible, 'mutator'));
    }
    /**
    ...
    updateWorkspace() {
        ...
        if (this.rootBlock && this.rootBlock.workspace === this.workspace_) {
            ...
            if (oldExtraState !== newExtraState) {
                eventUtils.fire(new (eventUtils.get(eventUtils.BLOCK_CHANGE))(block, 'mutation', null, oldExtraState, newExtraState));
