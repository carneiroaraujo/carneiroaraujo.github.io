...
export class RenderInfo extends BaseRenderInfo {
    ...
    getSpacerRowHeight_(prev, next) {
        ...
        if (Types.isBottomRow(next)) {
            ...
            if (!this.outputConnection) {
                const topHeight = Math.max(this.topRow.minHeight, Math.max(this.constants_.NOTCH_HEIGHT, this.constants_.CORNER_RADIUS)) -
    ...
    adjustXPosition_() {
        ...
        for (let i = 2; i < this.rows.length - 1; i += 2) {
            ...
            else if (hasPrevNotch && (i === 2 || hasNextNotch) && Types.isInputRow(row) &&
                !row.hasStatement) {
