...
class Bubble {
    ...
    constructor(workspace, content, shape, anchorXY, bubbleWidth, bubbleHeight) {
        ...
        this.onMouseDownBubbleWrapper = null;
        /** Mouse down on resizeGroup event data. */
        this.onMouseDownResizeWrapper = null;
        /**
        ...
        canvas.appendChild(this.createDom(content, !!(bubbleWidth && bubbleHeight)));
        this.setAnchorLocation(anchorXY);
        if (!bubbleWidth || !bubbleHeight) {
            const bBox = this.content_.getBBox();
    ...
    createDom(content, hasResize) {
        ...
        this.bubbleBack = dom.createSvgElement(Svg.RECT, {
        ...
        if (hasResize) {
            this.resizeGroup = dom.createSvgElement(Svg.G, {
                'class': this.workspace_.RTL ? 'blocklyResizeSW' :
                    'blocklyResizeSE',
            }, this.bubbleGroup);
            const size = 2 * Bubble.BORDER_WIDTH;
            dom.createSvgElement(Svg.POLYGON, { 'points': `0,${size} ${size},${size} ${size},0` }, this.resizeGroup);
        ...
        if (!this.workspace_.options.readOnly) {
            this.onMouseDownBubbleWrapper = browserEvents.conditionalBind(this.bubbleBack, 'pointerdown', this, this.bubbleMouseDown);
            if (this.resizeGroup) {
                this.onMouseDownResizeWrapper = browserEvents.conditionalBind(this.resizeGroup, 'pointerdown', this, this.resizeMouseDown);
            }
        }
    ...
    resizeMouseDown(e) {
        ...
        this.workspace_.startDrag(e, new Coordinate(this.workspace_.RTL ? -this.width : this.width, this.height));
        Bubble.onMouseUpWrapper = browserEvents.conditionalBind(document, 'pointerup', this, Bubble.bubbleMouseUp);
        Bubble.onMouseMoveWrapper = browserEvents.conditionalBind(document, 'pointermove', this, this.resizeMouseMove);
    ...
    getOverlap(relativeMin, viewMetrics) {
        ...
        return Math.max(0, Math.min(1, overlapWidth * overlapHeight / (this.width * this.height)));
    }
    /**
     ...
     */
    moveTo(x, y) {
        this.bubbleGroup?.setAttribute('transform', 'translate(' + x + ',' + y + ')');
    }
    /**
    ...
    setBubbleSize(width, height) {
        ...
        if (this.resizeGroup) {
            if (this.workspace_.RTL) {
                ...
                this.resizeGroup.setAttribute('transform', 'translate(' + resizeSize + ',' + (height - doubleBorderWidth) +
                    ') scale(-1 1)');
            }
            else {
                this.resizeGroup.setAttribute('transform', 'translate(' + (width - doubleBorderWidth) + ',' +
    ...
    renderArrow() {
        ...
        if (relBubbleX === relAnchorX && relBubbleY === relAnchorY) {
            ...
            let thickness = (bubbleSize.width + bubbleSize.height) / Bubble.ARROW_THICKNESS;
            ...
            steps.push('C' + relAnchorX + ',' + relAnchorY + ' ' + (baseX2 + swirlRun) +
    ...
    getRelativeToSurfaceXY() {
        return new Coordinate(this.workspace_.RTL ?
            -this.relativeLeft + this.anchorXY.x - this.width :
            this.anchorXY.x + this.relativeLeft, this.anchorXY.y + this.relativeTop);
    }
    /**
    ...
    static textToDom(text) {
        ...
        for (let i = 0; i < lines.length; i++) {
            const tspanElement = dom.createSvgElement(Svg.TSPAN, { 'dy': '1em', 'x': Bubble.BORDER_WIDTH }, paragraph);
    ...
    static createNonEditableBubble(paragraphElement, block, iconXY) {
        const bubble = new Bubble(block.workspace, paragraphElement, block.pathObject.svgPath, (iconXY), null, null);
        ...
        if (block.RTL) {
            ...
            for (let i = 0, textElement; textElement = paragraphElement.childNodes[i]; i++) {
