...
export function queueRender(block) {
    queueBlock(block);
    if (!pid)
        pid = window.requestAnimationFrame(doRenders);
}
/**
 ...
 */
function updateConnectionLocations(block, blockOrigin) {
    for (const conn of block.getConnections_(false)) {
        ...
        if (moved || dirtyBlocks.has(target)) {
            updateConnectionLocations(target, Coordinate.sum(blockOrigin, target.relativeCoords));
        }
    }
}
