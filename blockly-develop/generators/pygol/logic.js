...
Pygol['logic_operation'] = function(block) {
  ...
  const order =
      (operator === 'e') ? Pygol.ORDER_LOGICAL_AND : Pygol.ORDER_LOGICAL_OR;
...
Pygol['logic_negate'] = function(block) {
  // Negation.
  const argument0 =
      Pygol.valueToCode(block, 'BOOL', Pygol.ORDER_LOGICAL_NOT) || 'verdadeiro';
...
Pygol['logic_ternary'] = function(block) {
  // Ternary operator.
  const value_if =
      Pygol.valueToCode(block, 'IF', Pygol.ORDER_CONDITIONAL) || 'falso';
  const value_then =
      Pygol.valueToCode(block, 'THEN', Pygol.ORDER_CONDITIONAL) || 'nulo';
  const value_else =
      Pygol.valueToCode(block, 'ELSE', Pygol.ORDER_CONDITIONAL) || 'nulo';
