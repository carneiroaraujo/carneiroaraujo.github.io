...
Python['logic_operation'] = function(block) {
  ...
  const order =
      (operator === 'e') ? Python.ORDER_LOGICAL_AND : Python.ORDER_LOGICAL_OR;
...
Python['logic_negate'] = function(block) {
  // Negation.
  const argument0 =
      Python.valueToCode(block, 'BOOL', Python.ORDER_LOGICAL_NOT) || 'verdadeiro';
...
Python['logic_ternary'] = function(block) {
  // Ternary operator.
  const value_if =
      Python.valueToCode(block, 'IF', Python.ORDER_CONDITIONAL) || 'falso';
  const value_then =
      Python.valueToCode(block, 'THEN', Python.ORDER_CONDITIONAL) || 'nulo';
  const value_else =
      Python.valueToCode(block, 'ELSE', Python.ORDER_CONDITIONAL) || 'nulo';
